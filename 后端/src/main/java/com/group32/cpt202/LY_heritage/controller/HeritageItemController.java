...
@RequestMapping({
        "/heritages",
        "/ly-heritage/heritages",
        "/api/ly-heritage/heritages"
})
public class HeritageItemController {
    ...
    public HeritageDetailResponse addComment(@PathVariable Long id, @RequestBody MessageCreateRequest request) {
        heritageItemService.addComment(id, request.getUserId(), request.getContent(), request.getParentMessageId());
        return heritageItemService.getHeritageDetail(id);
    }
}
